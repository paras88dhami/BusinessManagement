...
import { PosMoneyAccountOption } from "../../types/pos.ui.types";
...
const INITIAL_STATE: PosScreenCoordinatorState = {
  status: Status.Idle,
  bootstrap: null,
  products: [],
  filteredProducts: [],
...
export type UsePosScreenEngineParams = {
...
export function usePosScreenEngine(
  params: UsePosScreenEngineParams,
) {
  const {
  ...
  const [state, setState] = useState<PosScreenCoordinatorState>(INITIAL_STATE);
  ...
  }, [printPosReceiptUseCase, state.receipt]);
...
export type PosScreenEngine = ReturnType<typeof usePosScreenEngine>;
